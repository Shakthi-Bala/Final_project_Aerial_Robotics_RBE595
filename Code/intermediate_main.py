...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dconv_up4(torch.cat([g, att4], dim=1))

        g = self.upsample(x)
        att3 = self.att3(g=g, x=conv3)
        x = self.dconv_up3(torch.cat([g, att3], dim=1))

        g = self.upsample(x)
        att2 = self.att2(g=g, x=conv2)
        x = self.dconv_up2(torch.cat([g, att2], dim=1))

        g = self.upsample(x)
        att1 = self.att1(g=g, x=conv1)
        x = self.dconv_up1(torch.cat([g, att1], dim=1))
...
########################################
def log_waypoint(t, pos, rpy=None, label=""):
    """
    Append a waypoint entry to the global log.
    We will only write label=='state' to CSV at the end.
    """
    global waypoint_log
    entry = {
        "t": float(t),
...
def yaw_in_place(currentPose,
    ...
    # 🔴 Removed logging here: we **do not** want yaw waypoints in CSV
...
def goToWaypoint(currentPose, targetPose,
    ...
    If log_path=False, states will not be added to waypoint_log.
    """
    ...
    # 🔴 Removed logging of the *target* itself
    # if log_path:
    ...
    for i, tt in enumerate(time_points):
        ...
        # ✅ Only log actual traversed states (if log_path=True)
        if log_path:
            log_waypoint(tt, current_pos, rpy=None, label="state")
...
def main(renderer):
    ...
    for k in range(warmup_cycles):
        ...
        # move LEFT (negative y) – 🔴 don't log warmup to waypoint_log
        ...
        # move RIGHT (positive y) – 🔴 don't log warmup
        cur_pos = currentPose['position'].copy()
    ...
    try:
        while not raceFinished:
            if (not all_window_passed) and (win_idx <= num_windows):
                ...
                # ✅ This is part of the forward run → keep log_path=True
                newPose = goToWaypoint(
                    currentPose,
                    targetPose1,
                    velocity=0.31,
                ...
                if win_idx == 1:
                    ...
                    sidePose = goToWaypoint(
                        ...
                        log_dir="./log",
                        frame_saver_thread=saver_thread,
                        log_path=True  # side shift is part of overall forward path
                    )
                ...
                if win_idx == 2 and not w2_done:
                    ...
                    target_side_1 = np.array([
                        cur_pos_2[0],
                        cur_pos_2[1] + side_offsets[win_idx],
                        cur_pos_2[2] - 0.1
                    ])
            ...
            if all_window_passed:
                # (unchanged RAFT part – only note where log_path is used)
                ...
                if hole_center is not None:
                    ...
                    # ✅ This is part of the final forward run → log_path=True
                    newPose = goToWaypoint(
                        currentPose,
                        target_hole,
                        velocity=0.4,
        ...
        # ----------------------------------------
        # Save **only forward-traversed waypoints** to CSV
        # ----------------------------------------
        global waypoint_log
        csv_path = os.path.join("./log", "waypoints.csv")
        os.makedirs("./log", exist_ok=True)

        # Filter to label=='state' (actual drone states in forward runs)
        state_entries = [e for e in waypoint_log if e.get("label", "") == "state"]

        if len(state_entries) > 0:
            fieldnames = sorted({k for entry in state_entries for k in entry.keys()})
            ...
            print(f"[INFO] Saved {len(state_entries)} forward waypoints to {csv_path}")
        else:
            print("[INFO] No 'state' waypoints logged; not writing CSV.")
