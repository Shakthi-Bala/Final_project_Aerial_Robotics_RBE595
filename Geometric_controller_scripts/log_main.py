...
def main(renderer):
    ...
    ray_flat[2] = 0.0  # no altitude change from ray

    norm_flat = np.linalg.norm(ray_flat)
    if norm_flat < 1e-6:
        forward_vec = np.array([1.0, 0.0, 0.0])
        print("[RAY] ray nearly vertical, using +X as forward")
    else:
        forward_vec = ray_flat / norm_flat

    # *** FIX 1: enforce that "forward" is always +X in world ***
    if forward_vec[0] < 0.0:
        forward_vec *= -1.0
        print(f"[RAY-FIX] Flipped forward_vec to keep +X forward: {forward_vec}")

    dist_forward = 0.3   # distance to move forward (tune as needed)
    targetPose1 = cam_origin_world + dist_forward * forward_vec

    # keep altitude same as current
    # *** FIX 2: move LEFT → negative y (since +y is right) ***
    targetPose1[1] += 0.15    # little left while going forward
    targetPose1[2] = cam_origin_world[2]
